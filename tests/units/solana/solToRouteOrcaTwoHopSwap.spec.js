...
describe('routeTwoHopOrcaSwap', ()=> {
  ...
  it('creates and closes a temp WSOL account through an twoHopOrcaSwap payment', async ()=>{
    ...
    expect(transaction.instructions[6].keys[13]).toEqual({ pubkey: new PublicKey('A6FdY6RXuHbrZvpt5ucefwDHvm99Vog9VUi1BQRfCny6'), isSigner: false, isWritable: true })
    expect(transaction.instructions[6].keys[14]).toEqual({ pubkey: new PublicKey('5ziJFbeTMgNLmRJYks622QmYcezESqU7UQkVAKLeKmEu'), isSigner: false, isWritable: true })
