...
describe('route wrapped', ()=> {
  ...
  it('provides payment route for NATIVE<>WRAPPED', async ()=>{
    ...
    expect(transaction.params.plugins[0]).toEqual(plugins[blockchain].wbnb.wrap.address)
  ...
  it('provides payment route for WRAPPED<>NATIVE', async ()=>{
    ...
    expect(transaction.params.addresses[0]).toEqual(fromAddress)
    expect(transaction.params.addresses[1]).toEqual(toAddress)
    expect(transaction.params.plugins[0]).toEqual(plugins[blockchain].wbnb.unwrap.address)
    expect(transaction.params.plugins[1]).toEqual(plugins[blockchain].payment.address)
    expect(routes[0].directTransfer).toEqual(false)
