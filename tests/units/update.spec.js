...
describe('update', ()=> {
  ...
  it('updates payment routes with intermediate results', async ()=>{
    ...
    transaction = await currentRoutes[0].getTransaction()
    ...
    expect(transaction.params.amounts[0]).toEqual(WBNB_USDT_bsc_amountInBN.add(WBNB_USDT_bsc_amountSlippageInBN).toString())
    ...
    transaction = await currentRoutes[1].getTransaction()
    ...
    expect(transaction.params.amounts[0]).toEqual(WETH_DAI_ethereum_amountInBN.add(WETH_DAI_ethereum_amountSlippageInBN).toString())
    ...
    expect(transaction.params.amounts[0]).toEqual(BUSD_bsc_amountInBN.add(BUSD_bsc_amountSlippageInBN).toString())
    expect(transaction.params.amounts[1]).toEqual(USDT_bsc_amount.toString())
    expect(transaction.params.addresses[0]).toEqual(fromAddress)
    expect(transaction.params.addresses[1]).toEqual(toAddress)
    expect(transaction.params.plugins[0]).toEqual(plugins.bsc.pancakeswap.address)
    ...
    transaction = await currentRoutes[2].getTransaction()
    expect(transaction.blockchain).toEqual('ethereum')
    ...
    transaction = await currentRoutes[3].getTransaction()
    expect(transaction.blockchain).toEqual('ethereum')
    expect(transaction.to).toEqual(routers.ethereum.address)
    expect(transaction.method).toEqual('route')
    expect(transaction.params.path).toEqual([Blockchains.ethereum.currency.address, DAI_ethereum])
    ...
    transaction = await currentRoutes[4].getTransaction()
    expect(transaction.blockchain).toEqual('ethereum')
    expect(transaction.to).toEqual(routers.ethereum.address)
    expect(transaction.method).toEqual('route')
    expect(transaction.params.path).toEqual([DEPAY, WETH, USDT_ethereum])
    expect(transaction.params.amounts[0]).toEqual('20100000')
    expect(transaction.params.amounts[1]).toEqual('20000000')
    expect(transaction.params.addresses[0]).toEqual(fromAddress)
    expect(transaction.params.addresses[1]).toEqual(toAddress)
    expect(transaction.params.plugins[0]).toEqual(plugins.ethereum.uniswap_v2.address)
    expect(transaction.params.plugins[1]).toEqual(plugins.ethereum.payment.address)
