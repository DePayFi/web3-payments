...
import { ACCOUNT_LAYOUT, Buffer, PublicKey, BN, struct, bool, i32, u64, u128, i128, seq, publicKey } from '@depay/solana-web3.js'
...
const exchange = Exchanges.orca.solana
  ...
  for (let i = 0; i < MAX_SWAP_TICK_ARRAYS; i++) {
    ...
    const pda = (
      await PublicKey.findProgramAddress([
        ...
        new PublicKey(exchange.router.address)
      )
    )[0]
...
async function mockPool({
  ...
  let data = Buffer.alloc(exchange.router.api.span)
  exchange.router.api.encode({
  ...
  mock({
    ...
    request: {
      ...
      params: { filters: [
        { dataSize: exchange.router.api.span },
      ...
      return: [{ account: { data, executable: false, lamports: 2039280, owner: exchange.router.address, rentEpoch: 327 }, pubkey: pool }]
    }
  })
...
async function mockPools({
  ...
  let poolTwoData = Buffer.alloc(exchange.router.api.span)
  exchange.router.api.encode({
    anchorDiscriminator: new BN("676526073106765119"),
    whirlpoolsConfig: new PublicKey("2LecshUwdy9xi7meFgHtFJQNSKk4KdTrcpvaB56dP2NQ"),
    whirlpoolBump: [255],
  ...
  mock({
    ...
    request: {
      ...
      return: [{ account: { data: poolTwoData, executable: false, lamports: 2039280, owner: exchange.router.address, rentEpoch: 327 }, pubkey: poolTwo }]
    }
  })
  ...
  let poolOneData = Buffer.alloc(exchange.router.api.span)
  exchange.router.api.encode({
    anchorDiscriminator: new BN("676526073106765119"),
    whirlpoolsConfig: new PublicKey("2LecshUwdy9xi7meFgHtFJQNSKk4KdTrcpvaB56dP2NQ"),
    whirlpoolBump: [255],
  ...
  mock({
    ...
    request: {
      ...
      return: [{ account: { data: poolOneData, executable: false, lamports: 2039280, owner: exchange.router.address, rentEpoch: 327 }, pubkey: poolOne }]
    }
  })
