...
describe('route to contract as payment receiver', ()=> {
  ...
  it('constructs payment transaction in a way that allows to pay into a smart contract with address, amount and boolean', async ()=>{
    ...
    expect(routes[1].transaction.params.plugins[0]).toEqual(plugins[blockchain].quickswap.address)
    ...
    expect(routes[2].transaction.params.plugins[0]).toEqual(plugins[blockchain].quickswap.address)
