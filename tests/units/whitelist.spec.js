...
describe('route', ()=> {
  ...
  it('whitelists accepted fromTokens and only calculates routes for those', async ()=>{
    ...
    expect(transaction.params.amounts[0]).toEqual(BUSD_bsc_amountInBN.add(BUSD_bsc_amountInSlippageInBN).toString())
    expect(transaction.params.amounts[1]).toEqual(USDT_bsc_amount.toString())
    expect(transaction.params.addresses[0]).toEqual(fromAddress)
    expect(transaction.params.addresses[1]).toEqual(toAddress)
    expect(transaction.params.plugins[0]).toEqual(plugins.bsc.pancakeswap.address)
    ...
    expect(transaction.params.amounts[0]).toEqual(WETH_DAI_ethereum_amountInBN.add(WETH_DAI_ethereum_amountInSlippageBN).toString())
